...
export const getAccountInfo = async (user, interaction) => {
    ...
    let accErr = accountData.error ? (Array.isArray(accountData.error) ? accountData.error[0].message : accountData.error) : null;
    if (accErr && typeof accErr === 'string' && accErr.includes("fetching needed match data")) {
...
export const fetchMatchHistory = async (interaction, user, mode = "competitive") => {
    ...
    if (mode === "competitive") {
        ...
        for (let i = 0; i < matchHistory.data.length; i++) {
            ...
            if (match.metadata.mode && match.metadata.mode.toLowerCase() !== mode.toLowerCase()) {
                console.log(`Skipping match ${i} - mode mismatch (expected ${mode}, got ${match.metadata.mode})`);
                continue;
            }
            ...
            // Safe rounds calculation (modes like Deathmatch don't have rounds)
